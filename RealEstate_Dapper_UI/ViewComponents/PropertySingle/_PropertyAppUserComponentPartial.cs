...
namespace RealEstate_Dapper_UI.ViewComponents.PropertySingle 
{
    public class _PropertyAppUserComponentPartial :ViewComponent
    {
        ...
        public async Task<IViewComponentResult> InvokeAsync()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:44382/api/Products/AppUsers?id=");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
                var values = JsonConvert.DeserializeObject<List<GetAppUserByProductIDDTO>>(jsonData);
                return View(values);
            }
            return View();
        }
